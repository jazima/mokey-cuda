...
void randomize_matrix_int(uint8_t *mat, int N) {
  // NOTICE: Use gettimeofday instead of srand((unsigned)time(NULL)); the time
  // precision is too low and the same random number is generated.
  struct timeval time {};
  gettimeofday(&time, nullptr);
...
void run_mokey_naive(int M, int N, int K, const uint8_t *A,
  ...
  dim3 blockDim(32, 32);
  mokey_naive<<<gridDim, blockDim>>>(M, N, K, A, B, C, sumA, sumB, exps, b, stdA, meanA, stdB, meanB);
}

void run_mokey_coalesce(int M, int N, int K, const uint8_t *A,
                        const uint8_t *B, float *C,
                        float *sumA, float *sumB, float *exps,
                        float b, float stdA, float meanA,
                        float stdB, float meanB) {
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32 * 32);
  mokey_global_mem_coalesce<32>
      <<<gridDim, blockDim>>>(M, N, K, A, B, C, sumA, sumB, exps, b, stdA, meanA, stdB, meanB);
}

void run_mokey_shared_mem_block(int M, int N, int K, float alpha, float *A,
...
void run_kernel(int kernel_num, int M, int N, int K, const uint8_t *A,
                const uint8_t *B, float *C,
                float *sumA, float *sumB, float *exps,
                float b, float stdA, float meanA,
                float stdB, float meanB) {
...
//     break;
  case 1:
    run_mokey_naive(M, N, K, A, B, C, sumA, sumB, exps, b, stdA, meanA, stdB, meanB);
    break;
  case 2:
    run_mokey_coalesce(M, N, K, A, B, C, sumA, sumB, exps, b, stdA, meanA, stdB, meanB);
    break;
//   case 3:
