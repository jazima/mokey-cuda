...
__global__ void mokey_global_mem_coalesce(int M, int N, int K, const uint8_t *A,
    ...
    if (cRow < M && cCol < N) {
        ...
        // C = α*(A@B)+β*C
        C[cRow * N + cCol] = stdA * stdB * (t1 + b * (t2 + b * count_sign)) + sumA*meanB +
                    sumB*meanA - len * meanA*meanB + C[cRow * N + cCol];
    }
}